...
namespace TwitterChallenge.Utility
{
    ...
    public static class TwitterExtensions
    {
        ...
        /// </summary>
        /// <param name="status">The LinqToTwitter status</param>
        ...
        public static string TextAsHtml(this Status status)
        {
            ...
            if (!String.IsNullOrEmpty(tweetText))
            {
                ...
                foreach (var hashMatch in _parseHashtags.Matches(tweetText))
                {
                    ...
                    tweetText = tweetText.Replace(match.Value, String.Format("<a href=\"https://twitter.com/search?q={0}\" target=\"_blank\">{1}</a>", query, match.Value));
                }
            }
